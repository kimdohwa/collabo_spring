...
public class GenerateData {
    //주의 } springboot가 아닌 일반 test모드에서는 @value를 사용할수 없습니다.
    public final String imageFolder = "C:/shop/images" ;

    //특정 폴더 내에 들어있는 모든 이미지 파일의 이름을 List 컬렉션으로 반환해 줍니다.
    public List<String> getImageFileMames(){

        //File 파일이나 폴더를 객체형태로 다루고자 할때 사용하는 클래스
        File folder = new File(imageFolder);
        List<String> imageFiles = new ArrayList<>();//이미지 이름들을 저장할 컬렉션

        //folder.exists() 해당 객체가 존재하면 트루
        //folder.isDirectory  해당 객체가 폴더이면 트루
        if(!folder.exists() || !folder.isDirectory()){
            System.out.println(imageFolder+"폴더가 존재하지않습니다.");
            return imageFiles;
        }

        String[] imageExtensions ={".jpg",".jpeg",".png"};  //관심있는 파일의 확장자
        File[] fileList =folder.listFiles();  //파일 객체목록

if(fileList != null){
    for (File file:fileList){
        if (file.isFile() && Arrays.stream(imageExtensions).anyMatch(ext -> file.getName().toLowerCase().endsWith(ext))){
